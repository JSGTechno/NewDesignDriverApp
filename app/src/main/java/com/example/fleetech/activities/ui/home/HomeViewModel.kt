...
class HomeViewModel : ViewModel() {
    ...
    val dutyDataList = MutableLiveData<DutyDataResponse>()
    val CheckInList = MutableLiveData<DutyDataResponse>()
...
//    }
    ...
    fun CheckInDriver(token: String, mobileNo: String, ctx: Context) {
        RetrofitClient.retrofit = null
        val userAPI = Apiuitils.testUrl
        println("MObile No $mobileNo")
        loading.value = true
        val call = userAPI.drivrcheckInDetails("Bearer " + token,mobileNo)
        call.enqueue(object : Callback<DutyDataResponse> {
            override fun onResponse(
                call: Call<DutyDataResponse>,
                response: Response<DutyDataResponse>
            ) {
                loading.value = false

                if (response.code() == 200) {
                    CheckInList.postValue(response.body())
    ...
    fun DutyDriver(token: String, mobileNo: String, latitude: String, longitude: String, address: String) {
        RetrofitClient.retrofit = null
        val userAPI = Apiuitils.testUrl
        println("MObile No $mobileNo")
        loading.value = true
        val call = userAPI.drivrDuty("Bearer " + token,DriverDutyModel(mobileNo, latitude, longitude, address))
        call.enqueue(object : Callback<DutyDataResponse> {
            override fun onResponse(
                call: Call<DutyDataResponse>,
                response: Response<DutyDataResponse>
               ...
               // Log.i("TAG","data_duty" + response.body());
                loading.value = false

                if (response.code() == 200) {
                    dutyDataList.postValue(response.body())
                    Log.i("TAG","data_duty" + response.body());
